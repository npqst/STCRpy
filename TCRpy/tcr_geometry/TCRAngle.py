...
class TCRAngle():
    ...
    def _get_TCR_chain_ids(self):
        raise NotImplementedError('TCRAngle is a base class. Instantiate a descendent class, eg. abTCRAngle')
    ...
    def _get_reference_atoms(self):
        ...
        for chain_id in self.chain_types:
            ...
            reference_structure = parser.get_tcr_structure(f'ref_{chain_id}', reference_pdb_file)
            reference_atoms[chain_id] = {
                    atom.parent.id[1]: atom
                    for atom in reference_structure.get_atoms()
                }
    ...
    def _get_reference_vectors(self):
        ...
        self.position_vectors = {
            chain_id:
            np.asarray(reference_data.reference_positions[chain_id])
            for chain_id in self.chain_types
            }
        ...
        self.plane_1 = {
            chain_id:
            self.plane_origin[chain_id] + np.asarray(self.position_vectors[chain_id][0])
            for chain_id in self.chain_types
        }
        self.plane_2 = {
            chain_id:
            self.plane_origin[chain_id] + np.asarray(self.position_vectors[chain_id][1])
            for chain_id in self.chain_types
        }
    ...
    def calculate_points(self, tcr, chain_id):
        ...
        coordinates = np.asarray([atom.get_coord() for res_id, atom in tcr_atoms.items()])
    ...
    def calculate_angles(self, tcr):
        ...
        for chain_type in self.chain_types:
            plane_origin[chain_type], plane_1[chain_type], plane_2[chain_type] = self.calculate_points(tcr, chain_type)

        # Create vectors with which to calculate angles between.
        C = self._normalise(plane_origin[self.chain_types[1]] - plane_origin[self.chain_types[0]])
        Cminus = [-1 * x for x in C]
        A1 = self._normalise(plane_1[self.chain_types[0]] - plane_origin[self.chain_types[0]])
        A2 = self._normalise(plane_2[self.chain_types[0]] - plane_origin[self.chain_types[0]])
        B1 = self._normalise(plane_1[self.chain_types[1]] - plane_origin[self.chain_types[1]])
        B2 = self._normalise(plane_2[self.chain_types[1]] - plane_origin[self.chain_types[1]])
        dc = np.sqrt(((plane_origin[self.chain_types[1]] - plane_origin[self.chain_types[0]]) ** 2).sum())
        ...
        return dict(
            list(
                zip(
                    [f"{self.chain_types[1]}{self.chain_types[0]}", f"{self.chain_types[1]}C1", f"{self.chain_types[0]}C1", f"{self.chain_types[1]}C2", f"{self.chain_types[0]}C2", "dist"],
    ...
    def get_angles(self, tcr):
        ...
        try:
            return {
                t.id: self.calculate_angles(t) for t in tcr.get_TCRs()
            }
