...
class TCRpMHC_PLIP_Model_Parser():
    ...
    def parse_tcr_pmhc_complex(
        ...
        assert len(peptide_chain) == 1, f'More or less than one peptide chain found: {peptide_chain}'
        ligand.add(peptide_chain[0])

        tcr_and_mhc_chains = list(tcr_pmhc_complex.get_chains()) + list(tcr_pmhc_complex.get_MHC()[0].get_chains())
        if renumber:
        ...
        if delete_tmp_files:
            ...
            if renumber:
                return complex, TCR_MHC_FILE, PEPTIDE_PDB_FILE, PEPTIDE_SDF_FILE, renumbering, domain_assignment
            else:
    ...
    def map_amino_acids_to_ligands(self, ligand_pdb, ligand_sdf):
        ...
        for coord in sdf_coords:
            pdb_atom = [a for a in ligand_structure.get_atoms() if sum((a.coord - coord)**2) < 0.0001]
            assert len(pdb_atom) == 1
            coord_to_aa[tuple(coord)] = (pdb_atom[0].parent.resname, pdb_atom[0].parent.id[1])
        return coord_to_aa