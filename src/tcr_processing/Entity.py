...
class Entity(Bio.PDB.Entity.Entity):
    ...
    def save(self,output=sys.stdout,renumber=True,selection=False,remarks=True):
        ...
        if remarks:
            remark_string=("REMARK   5 IMGT RENUMBERED STRUCTURE %s GENERATED BY STCRDAB\n" % str(self.id).upper())
            remark_string+="REMARK   5 TCR CHAINS ARE RENUMBERED IN THE VARIABLE REGIONS ONLY\n"
            ...
            while i<6: # only try and go up residue,chain, holder, model, structure
                if hasattr(p, "warnings"):
                   ...
                   for TCR in p.get_TCRs():
                       ...
                       remark_string+= "REMARK   5 PAIRED_%s %sCHAIN=%s %sCHAIN=%s MHCCHAINS=%s AGCHAIN=%s AGTYPE=%s\n"%(TCR.get_TCR_type(), ch0[0], ch0[1], ch1[0], ch1[1], mh_chains, antigen_chain, antigen_type) 

                   for TR_chain in p.get_unpaired_TCRchains():
                       antigen_chain, antigen_type = ag_chain_and_type(TR_chain.antigen)
                       if TR_chain.get_MHC():
                           mh_chains = TR_chain.get_MHC()[0].get_id()
                       else:
                           mh_chains = ""
                       remark_string+= "REMARK   5 SINGLE %sCHAIN=%s MHCCHAINS=%s AGCHAIN=%s AGTYPE=%s\n"%(TR_chain.chain_type, TR_chain.id, mh_chains, antigen_chain, antigen_type)
