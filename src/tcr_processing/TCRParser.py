...
class TCRParser():
    ...
    def _create_chain(self, chain, new_chain_id, numbering, chain_type):
        ...
        elif chain_type in ["MH1", "CD1", "B2M", "GA", "GB", "GA1L", "GA2L", "GA1", "GA2", "MR1"]:
    ...
    def _create_scTCR_chains(self, chain, new_chain_id, numbering_1, numbering_2, chain_type1, chain_type2):
        """
        ...
        if chain_type2 in ["G", "A"]:
            # Just a trick to 
            return self._create_scTCR_chains( chain, new_chain_id, numbering_2, numbering_1, chain_type2, chain_type1)
    ...
    def get_tcr_structure(self, id, file, prenumbering=None, ali_dict = {}, crystal_contacts=[]):
        """
        ...
        for mid in range(len(structure.child_list) - 1, -1, -1): 
            ...
            for chain in model.get_list():
                ...
                if prenumbering and chain.id in prenumbering:
                    if len(prenumbering[chain.id])==2:
                        ...
                        numbering[0], region_types[0] = self._prenumbered(chain, prenumbering, ali_dict, n = 0)
                        numbering[1], region_types[1] = self._prenumbered(chain, prenumbering, ali_dict, n = 1)
                        ...
                        numbering, chain_type = self._prenumbered(chain, prenumbering, ali_dict, n=0)
                        scTCR = False
                ...
                if numbering and chain_type in ["G", "D", "B", "A"]:
                    ...
                    newchain = self._create_chain(chain, chain.id,numbering, chain_type)
                    newchain.set_engineered(engineered)
                    newchain.xtra.update(details)
                    trchains.add(newchain)

                elif numbering and chain_type in ["MH1", "CD1", "GA", "GB", "B2M", "GA1L", "GA2L", "GA1", "GA2", "MR1"]:
                    newchain = self._create_chain(chain, chain.id, numbering, chain_type)
                    newchain.set_engineered(engineered)
                    newchain.xtra.update(details)
                    ...
                    chain1, chain2 = self._create_scTCR_chains(chain, chain.id, domain1, domain2, types[0], types[1])
                    ...
                    if (chain1.child_dict[(" ", 104, ' ')]['CA'] - chain2.child_dict[(" ", 104, ' ')]["CA"]) <= 22:
            ...
            if not scTCR:
                ...
                for pair in pairings:
                    ...
                    if not obs_chaintypes - set(["A", "B"]):
                        ...
                        sys.stderr.write("Unusual pairing between %s (V%s) and %s (V%s) has been detected. Treating as separate TCR chains.\n" % (pair[0].id, pair[0].chain_type, pair[1].id, pair[1].chain_type))
                        trchains.add(pair[0])
                        trchains.add(pair[1])
                        continue
                    newmodel.add(tcr)
            ...
            for pair in pairings:
                ...
                obs_chaintypes = set([pair[0].chain_type, pair[1].chain_type])
                if not (obs_chaintypes - set(["MH1", "B2M"])) or not (obs_chaintypes - set(["GA1", "GA2"])) or not (obs_chaintypes - set(["GA1", "B2M"])):
                    mhc = MH1(pair[0],pair[1])
                elif not (obs_chaintypes - set(["GA", "GB"])):
                    mhc = MH2(pair[0],pair[1])
                elif not (obs_chaintypes - set(["CD1", "B2M"])) or not (obs_chaintypes - set(["GA1L", "GA2L"])):
            ...
            empty_holders = [ holder.id for holder in newmodel.child_list if not holder.child_list ]
    ...
    def _pair_mhc(self, chains):
        ...
        acceptable_types = ["MH1:B2M", "GB:GA", "CD1:B2M", "MR1:B2M", "GA1:B2M", "GA1:B2M"]
        ...
        for pair in combinations(chains, 2):
            ...
            if the_type in acceptable_types:
                ...
                try:
                    a1, a2 = p1[points[p1.chain_type][0]]['CA'], p1[points[p1.chain_type][1]]['CA']
                    a3, a4 = p2[points[p2.chain_type][0]]['CA'], p2[points[p2.chain_type][1]]['CA']
                    dist_array = [ a3-a1, a4-a2, a4-a1, a3-a2 ]
                    constants  = all([dist_array[i] <= MHC_CUTOFF[the_type][i] for i in range(len(dist_array))])
                    if constants:                        
                        pairings.append(pair)
        ...
        for c in [ l.strip() for l in  self.trailer if "CONECT" in l]:
            ...
            for b,e in [ (11,16), (16,21), (21,26), (26, 31) ]:
                try: 
                    ...
                    self.warnings.write("Warning: unexpected CONECT record format %s"%l.strip())
    ...
    def _match_units(self, model, trchains, mhchains, agchains, crystal_contacts=[]):
        ...
        antigen_atoms, cdr_atoms, mh_atoms, antigen_hetatoms, antigen_sugars = defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list)
        ...
        for tr in tcell_receptors:
            for cdr in tr.get_CDRs():
                ...
                cdr_atoms[tr.id] += [atom for atom in cdr.get_atoms() if atom.id=="CB" or atom.id=="CA"]
            ...
            if isinstance(tr, TCR) and tr.get_TCR_type() == "abTCR":
                ...
                antigen_hetatoms[tr.VB], antigen_sugars[tr.VB] = self._find_chain_hetatoms(beta_chain)
                antigen_hetatoms[tr.VA], antigen_sugars[tr.VA] = self._find_chain_hetatoms(alpha_chain)

            elif isinstance(tr, TCR) and tr.get_TCR_type() == "gdTCR":
                delta_chain = tr.get_VD()
                gamma_chain = tr.get_VG()
                antigen_hetatoms[tr.VD], antigen_sugars[tr.VD] = self._find_chain_hetatoms(delta_chain)
                antigen_hetatoms[tr.VG], antigen_sugars[tr.VG] = self._find_chain_hetatoms(gamma_chain)

            # Unpaired TCR chain
            elif isinstance(tr, TCRchain):
                antigen_hetatoms[tr.id], antigen_sugars[tr.id] = self._find_chain_hetatoms(tr)

        # Do the same for MHC.
        for mh in mhc_complexes:
            # Keep G domain atoms; Get the Helix region of MHC
            mh_atoms[mh.id] = [ atom for atom in mh.get_atoms() if (atom.id == "CB" or atom.id == "CA") and atom.region == "Helix" ] 
            if isinstance(mh, MHC) and mh.MHC_type == "MH1":
                MH1, B2M = mh.get_MH1(), mh.get_B2M()
                if MH1 is not None:
                    antigen_hetatoms[mh.MH1], antigen_sugars[mh.MH1] = self._find_chain_hetatoms(MH1)
                else:
                    GA1 = mh.get_GA1()
                    antigen_hetatoms[mh.GA1], antigen_sugars[mh.GA1] = self._find_chain_hetatoms(GA1)
                antigen_hetatoms[mh.B2M], antigen_sugars[mh.B2M] = self._find_chain_hetatoms(B2M)

            elif isinstance(mh, MHC) and mh.MHC_type == "CD1":
                CD1, B2M = mh.get_CD1(), mh.get_B2M()
                antigen_hetatoms[mh.CD1], antigen_sugars[mh.CD1] = self._find_chain_hetatoms(CD1)
                antigen_hetatoms[mh.B2M], antigen_sugars[mh.B2M] = self._find_chain_hetatoms(B2M)
            
            elif isinstance(mh, MHC) and mh.MHC_type == "MR1":
                MR1, B2M = mh.get_MR1(), mh.get_B2M()
                antigen_hetatoms[mh.MR1], antigen_sugars[mh.MR1] = self._find_chain_hetatoms(MR1)
                antigen_hetatoms[mh.B2M], antigen_sugars[mh.B2M] = self._find_chain_hetatoms(B2M)

            elif isinstance(mh, MHC) and mh.MHC_type == "MH2":
                GA, GB = mh.get_GA(), mh.get_GB()
                antigen_hetatoms[mh.GA], antigen_sugars[mh.GA] = self._find_chain_hetatoms(GA)
                antigen_hetatoms[mh.GB], antigen_sugars[mh.GB] = self._find_chain_hetatoms(GB)
                
            # Unpaired MHC chains -- if any, go here.
            elif isinstance(mh, MHCchain):
                antigen_hetatoms[mh.id], antigen_sugars[mh.id] = self._find_chain_hetatoms(mh)
        ...
        for chain_id in antigen_sugars:
            if antigen_sugars[chain_id]:
                polymers, monomer_atoms =  self._get_sugar_fragments(antigen_sugars[chain_id])
                sugars += polymers
        ...
        self._protein_peptide_pass(model, tcell_receptors, cdr_atoms, antigen_atoms, crystal_contacts)
        self._het_sugar_pass(tcell_receptors, cdr_atoms, non_empty_ag, antigen_hetatoms, sugars, distance=8.0)
        ...
        self._het_sugar_pass(mhc_complexes, mh_atoms, non_empty_ag, antigen_hetatoms, sugars, distance = 3.5)

        if antigen_atoms:
            self._protein_peptide_pass(model, mhc_complexes, mh_atoms, antigen_atoms, crystal_contacts)
        ...
        sorted_contacts = sorted(list(contact_freq.items()), key = lambda z: z[1], reverse = True)
        paired_tr_mh = set()
    ...
    def _protein_peptide_pass(self, model, complexes, receptor_atoms, antigen_atoms, crystal_contacts = []):
        """
        ...
        """
        ns = NeighborSearch( [ atom for chain in receptor_atoms for atom in receptor_atoms[chain] ] + [atom for chain in antigen_atoms for atom in antigen_atoms[chain]]  )
        ...
        for c in contacts:
            ...
            if p1 == p2 or potential_contact in contact_freq or potential_contact2 in contact_freq:
                continue
            ...
            elif (potential_contact2 not in cpxids) and (p2 in all_cpx_chains) and (p1 not in all_cpx_chains):
                T = all_cpx_chains[p2]
                ag = p1
            else:
                continue
        ...
        for cpx_id in cpxids:
            ...
            if contact_freq[cpx_id]:
                ...
                if (cpx_id, ag) not in crystal_contacts:
                    model[cpx_id].antigen = [] # disregard smaller antigens if peptide or protein present.
                    model[cpx_id]._add_antigen( model[ag] ) # pair up an antigen to the TCR.
        ...
        for ag in ags:
            ...
            if cmax:
                if len(contact_freq)>1:
                    self.warnings.write("Crystal Contact Warning: antigen %s has been paired with TCR %s"%( str(ag), str(paired_cpx)) )
                    model[paired_cpx]._add_antigen( model[ag] )
                else:
                    model[paired_cpx]._add_antigen( model[ag] )


    def _het_sugar_pass(self, receptors, receptor_atoms, non_empty_ag, antigen_hetatoms, sugars, distance = 8.0):
        """
        """
                    ...
                    for contact in contacts:
                        ...
                        if self._check_het_antigen(contact):
                            ...
                            if residue_type=="Hapten":
                                self.warnings.write("Warning: Multiple hapten-antigen like molecules found in binding site - this needs attention as could be solvent/cofactor.")
    ...
    def _check_het_antigen(self,residue):
        ...
        if is_common_buffer(residue):
            if not self.QUIET:
                self.warnings.write("Common molecule %s found in the binding site - not considered an antigen"%residue.get_resname())
            return False
