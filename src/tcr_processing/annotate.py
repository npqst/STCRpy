...
def call_anarci( seq, allow = set( ["B","A","D","G", "GA1", "GA2", "GA1L", "GA2L", "GA", "GB", "B2M", "MH1", "MR1","MR2"] ) ):
    """
...
def annotate(chain):
    ...
    if chtype in ("BA", "GD"):
        aligned_numbering = align_scTCR_numbering(numbering, sequence_list, sequence_str)
            ...
            if residue.get_resname() in to_one_letter_code:
                if ignore_hets:
                    if return_warnings:
                        ...
                        sys.stderr.write("Warning: HETATM residue %s position %s (PDB numbering) found in chain %s. Not including it in structure's sequence.\n"%(residue.get_resname(), str(residue.id[1])+residue.id[2].strip(), residue.parent.id ))
                    continue
            else:
                continue
...
def align_numbering(numbering, sequence_list,alignment_dict={}):
    ...
    if numbering:
        ...
        if not alignment_dict:
            try:
                numbered_sequence_ali,input_sequence_ali = pairwise_alignment(numbered_sequence, input_sequence)
                alignment_dict = get_alignment_dict(input_sequence_ali,numbered_sequence_ali)
            except:
                raise Exception("Could not align numbered sequence to aligned sequence:"+" "+str(numbered_sequence)+" "+str(input_sequence))
...
def align_scTCR_numbering(numbering, sequence_list, sequence_str):
    ...
    if numbering:
        ...
        for ii, a_sequence in enumerate(numbered_sequence):
            ...
            try:
                a_sequence_ali, input_sequence_ali = pairwise_alignment(a_sequence, input_sequence)
            ...
            for i in alignment_dict:
                if i in alignment_dict:
                    #during
                    assert after_flag is False, "Extra residue in structure than expected from provided sequence"
                    assert input_sequence[i] == numbered_sequence[ii][alignment_dict[i]], "alignment dictionary failed"
                    aligned_numbering[ii][sequence_list[i][0]] = numbering[ii][alignment_dict[i]][0]
        ...
        if key not in numbering_dict:
            ...
            while not left:
                ...
                if key_left in numbering_dict:
                    left = True
                    lidx = index - j # Last known index of sequence_list where we know a key exists
            ...
            while not right:
                ...
                if key_right in numbering_dict:
                    right = True
                    ridx  = index + j # Last known index of sequence_list on the right where we know a key exists
                else:
                    j += 1
