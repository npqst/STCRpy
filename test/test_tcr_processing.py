...
class TestTCRParser(unittest.TestCase):
    ...
    def test_save(self):
        ...
        for x in tcr.get_TCRs():
            io.save(x, save_as=f"TCRpy/test/test_files/test_{x.id}_TCR_only.pdb")

        for x in tcr.get_TCRs():
            io.save(x, tcr_only=True, save_as=f"TCRpy/test/test_files/test_{x.id}.pdb")
        ...
        tcr = parser.get_tcr_structure("test", pdb_file)
        for x in tcr.get_TCRs():
            io.save(x, save_as=f"TCRpy/test/test_files/test_{x.id}.pdb")
